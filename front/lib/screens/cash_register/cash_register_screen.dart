                ...
                return RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return _RegisterHistoryCard(reg: reg);
                    },
                  ),
                );
              },
...
class _TodayRegisterStatusHeader extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (isOpen) ...[
            Text('Opened: ${Formatters.dateTime(reg!.openedAt)}',
  ...
  void _showOpenRegisterDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (ctx) => _OpenRegisterDialog(ref: ref),
    );
  }
  ...
  void _showCloseRegisterDialog(BuildContext context, CashRegister reg) {
    showDialog(
      context: context,
      builder: (ctx) => _CloseRegisterDialog(reg: reg, ref: ref),
    );
  }
}
...
class _OpenRegisterDialogState extends State<_OpenRegisterDialog> {
      ...
      content: Form(
        ...
        child: TextFormField(
          ...
          decoration: const InputDecoration(
            labelText: 'Opening Cash Balance (DT)',
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await widget.ref.read(cashRegisterNotifierProvider.notifier).openRegister({
        'business': businessId,
        'openingBalance': Validators.parseDouble(_amountCtrl.text),
      });
      ...
      if (mounted) {
        ...
        AppSnackbar.success(context, 'Register opened successfully');
      }
    } catch (e) {
    ...
    return AlertDialog(
      ...
      content: Form(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Opening Balance: ${Formatters.currency(widget.reg.openingBalance)}',
                style: AppTypography.bodySmall),
            const SizedBox(height: AppSpacing.md),
            TextFormField(
              controller: _amountCtrl,
              decoration: const InputDecoration(
                labelText: 'Actual Closing Cash (DT)',
                prefixIcon: Icon(Icons.attach_money),
                hintText: '0.000',
              ),
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
  ...
  Future<void> _submit() async {
    ...
    try {
      await widget.ref
          ...
          .closeRegister(widget.reg.id, Validators.parseDouble(_amountCtrl.text));

      widget.ref.invalidate(todayCashRegisterProvider);

      if (mounted) {
        Navigator.pop(context);
        AppSnackbar.success(context, 'Register closed successfully');
      }
    } catch (e) {
      if (mounted) AppSnackbar.error(context, e.toString());
    } finally {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: _MiniItem(label: 'Opening', value: Formatters.currency(reg.openingBalance)),
                ),
                if (reg.closingBalance != null)
                  Expanded(
                    child: _MiniItem(label: 'Closing', value: Formatters.currency(reg.closingBalance!)),
                  ),
                if (reg.expectedBalance != null)
                  Expanded(
                    child: _MiniItem(label: 'Expected', value: Formatters.currency(reg.expectedBalance!)),
                  ),
              ],
            ),
